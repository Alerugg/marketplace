...
const defaultListingPayload = {
  print_id: 'print_vendor_order_actions_001',
  price_amount: 19.99,
  currency_code: 'eur',
  ...
  location_country: 'ES',
  shipping_profile_id: 'sp_vendor_order_actions_123'
}
  ...
  testSuite: ({ api, getContainer }) => {
    ...
    const createSellerAuthContext = async (
      ...
      const seller = await sellerService.createSellers({
        name: sellerName,
        handle: `vendor-order-actions-seller-${unique}`,
    ...
    const setupSellerMarketplace = async (
      ...
      await createSellerShippingProfile(
        seller,
        `Vendor Order Actions Shipping Profile ${sellerLabel} ${unique}`
      )
    ...
    const createProductVariantId = async (
      ...
      const {
        ...
        input: {
          products: [
            {
              title: `Vendor Order Actions Product ${unique}`,
              handle: `vendor-order-actions-product-${unique.replace(/_/g, '-')}`,
              status: 'published',
              sales_channels: [
                {
                  id: salesChannelId
    ...
    const createCart = async (setup: MarketplaceSetup) => {
      ...
      const created = await cartService.createCarts({
        ...
        email: `buyer-vendor-order-actions-${unique}@example.com`,
    ...
    const completeCartWithListingAndReturnOrderId = async (
      ...
      const listing = await createListing(setup, {
        print_id: `print_vendor_order_actions_${Date.now()}_${Math.floor(
          Math.random() * 10000
        )}`,
      ...
      const addListingResponse = await api.post(
        `/store/carts/${cart.id}/listings`,
        {
          ...
          quantity: 1,
          metadata: {
            source: 'vendor-order-actions-test'
          }
        },
        {
    ...
    describe('Vendor Order Actions API', () => {
      ...
      it('allows the authenticated seller to cancel their own order', async () => {
        const response = await api.post(
          `/vendor/orders/${sellerAOrderId}/cancel`,
          {},
          {
            headers: {
        ...
        expect(response.data.order?.status).toBe('canceled')
      })

      it('allows the authenticated seller to complete their own order', async () => {
        const response = await api.post(
          `/vendor/orders/${sellerAOrderId}/complete`,
          {},
          {
            headers: {
              Authorization: `Bearer ${sellerASetup.seller.token}`
      ...
      it('rejects canceling an order that belongs to another seller', async () => {
        const response = await api.post(
          `/vendor/orders/${sellerBOrderId}/cancel`,
          {},
          {
            headers: {
              Authorization: `Bearer ${sellerASetup.seller.token}`
      ...
      it('rejects completing an order that belongs to another seller', async () => {
        const response = await api.post(
          `/vendor/orders/${sellerBOrderId}/complete`,
          {},
          {
            headers: {
              Authorization: `Bearer ${sellerASetup.seller.token}`
      ...
      it('requires authentication on cancel and complete routes', async () => {
        ...
        expect([401, 403]).toContain(completeResponse.status)
      })
    })
  }
})
