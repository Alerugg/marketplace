...
medusaIntegrationTestRunner({
  ...
  testSuite: ({ api, getContainer }) => {
    ...
    describe('Vendor Listings API', () => {
      ...
      it('allows only one decrement to succeed when concurrent decrements compete for the last unit', async () => {
        const listingModuleService = getContainer().resolve<any>('listing')

        const activeListing = await listingModuleService.createListings({
          ...defaultListingPayload,
          print_id: `print_domain_atomic_compete_${Date.now()}`,
          seller_id: sellerA.sellerId,
          status: 'active',
          quantity_available: 1
        } as any)
      ...
      it('rejects repeated sells for sold listings via listing service', async () => {
        const listingModuleService = getContainer().resolve<any>('listing')

        const activeListing = await listingModuleService.createListings({
          ...defaultListingPayload,
          print_id: `print_domain_repeated_sell_${Date.now()}`,
          seller_id: sellerA.sellerId,
          status: 'active',
          quantity_available: 1
        } as any)

        await listingModuleService.sellListing(activeListing.id)

        await expect(
          listingModuleService.sellListing(activeListing.id)
        ).rejects.toMatchObject({
          type: MedusaError.Types.INVALID_DATA,
          message: expect.stringContaining('sold listings cannot be mutated')
        })
