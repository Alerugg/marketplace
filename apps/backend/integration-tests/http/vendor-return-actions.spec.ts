...
const defaultListingPayload = {
  print_id: 'print_vendor_returns_query_001',
  price_amount: 19.99,
  currency_code: 'eur',
  condition_code: 'near_mint',
  quantity_available: 1,
  status: 'active',
  seller_note: 'vendor returns query test listing',
  photos: ['https://example.com/a.jpg'],
  location_country: 'ES',
  shipping_profile_id: 'sp_vendor_returns_query_123'
}
  ...
  testSuite: ({ api, getContainer }) => {
    ...
    const createSellerAuthContext = async (
      ...
      const seller = await sellerService.createSellers({
        name: sellerName,
        handle: `vendor-returns-query-seller-${unique}`,
    ...
    const createUniqueServiceZoneForFulfillmentSet = async (
      ...
      const {
        ...
        input: {
          data: [
            {
              name: `Europe Vendor Returns ${sellerLabel} ${unique}`,
    ...
    const setupSellerMarketplace = async (
      ...
      const seller = await createSellerAuthContext(
        `seller-vendor-returns-${sellerLabel}-${unique}@example.com`,
        `Seller Vendor Returns ${sellerLabel} ${unique}`
      )

      await createSellerShippingProfile(
        seller,
        `Vendor Returns Shipping Profile ${sellerLabel} ${unique}`
      )
    ...
    const createProductVariantId = async (
      ...
      const {
        ...
        input: {
          products: [
            {
              title: `Vendor Returns Product ${unique}`,
              handle: `vendor-returns-product-${unique.replace(/_/g, '-')}`,
              status: 'published',
              sales_channels: [
                {
                  id: salesChannelId
    ...
    const createCart = async (
      ...
      const created = await cartService.createCarts({
        ...
        email: customer.email || `buyer-vendor-returns-${unique}@example.com`,
    ...
    const completeCartWithListingAndReturnOrderId = async (
      ...
      const listing = await createListing(setup, {
        print_id: `print_vendor_returns_query_${Date.now()}_${Math.floor(
          Math.random() * 10000
        )}`,
      ...
      const addListingResponse = await api.post(
        `/store/carts/${cart.id}/listings`,
        {
          ...
          quantity: 1,
          metadata: {
            source: 'vendor-returns-query-test'
          }
        },
        {
    ...
    describe('Vendor Return Actions Ownership API', () => {
      ...
      let sellerAReturnId: string
      let sellerBReturnId: string

      type ActionRequestCase = {
        method: 'post' | 'delete'
        path: string
      ...
      const validItemsPayload = {
        items: [
          {
            id: 'item_vendor_return_actions_dummy',
            quantity: 1
          }
        ]
      }

      const actionCases = (returnId: string): ActionRequestCase[] => [
        {
          method: 'post',
          path: `/vendor/returns/${returnId}/receive`,
          body: {}
        },
        {
          method: 'post',
          path: `/vendor/returns/${returnId}/receive/confirm`,
          body: {}
        },
        {
          method: 'post',
          path: `/vendor/returns/${returnId}/receive-items`,
          body: validItemsPayload
        },
        {
          method: 'post',
          path: `/vendor/returns/${returnId}/receive-items/action_foreign`,
          body: {
            quantity: 1
          }
        },
        ...
        {
          method: 'post',
          path: `/vendor/returns/${returnId}/dismiss-items`,
          body: validItemsPayload
        },
        {
          method: 'post',
          path: `/vendor/returns/${returnId}/dismiss-items/action_foreign`,
          body: {
            quantity: 1
          }
        },
      ...
      beforeEach(async () => {
        ...
        sellerASetup = await setupSellerMarketplace(
          marketplaceBase,
          `A actions ${uniqueSeed}`
        )

        sellerBSetup = await setupSellerMarketplace(
          marketplaceBase,
          `B actions ${uniqueSeed}`
        )

        customerA = await createCustomerAuthContext(
          `customer-a-vendor-return-actions-${uniqueSeed}@example.com`
        )

        customerB = await createCustomerAuthContext(
          `customer-b-vendor-return-actions-${uniqueSeed}@example.com`
        )
        ...
        sellerAReturnId = await createReturnForOrder(
          sellerAOrderId,
          sellerASetup.seller.sellerId
        )

        sellerBReturnId = await createReturnForOrder(
          sellerBOrderId,
          sellerBSetup.seller.sellerId
        )
      })

      it('rejects return action routes when the return belongs to another seller', async () => {
        for (const actionCase of actionCases(sellerBReturnId)) {
          const response = await requestAction(
            actionCase,
            sellerASetup.seller.token
          )
      ...
      it('requires authentication on return action routes', async () => {
        for (const actionCase of actionCases(sellerAReturnId)) {
          const response = await requestAction(actionCase)

          if (![401, 403].includes(response.status)) {
            console.log('Unexpected unauthenticated action response:', {
              method: actionCase.method,
              path: actionCase.path,
              status: response.status,
              data: response.data
