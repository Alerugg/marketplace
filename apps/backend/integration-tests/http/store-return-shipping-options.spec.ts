...
const defaultListingPayload = {
  print_id: 'print_store_return_shipping_options_001',
  price_amount: 19.99,
  currency_code: 'eur',
  condition_code: 'near_mint',
  quantity_available: 1,
  status: 'active',
  seller_note: 'store return shipping options test listing',
  photos: ['https://example.com/a.jpg'],
  location_country: 'ES',
  shipping_profile_id: 'sp_store_return_shipping_options_123'
}
  ...
  testSuite: ({ api, getContainer }) => {
    ...
    const createSellerAuthContext = async (
      ...
      const seller = await sellerService.createSellers({
        name: sellerName,
        handle: `store-return-shipping-options-seller-${unique}`,
    ...
    const createSellerShippingProfile = async (
      ...
      const response = await api.post(
        ...
        {
          name,
          type: 'store_return_shipping_options'
        },
        {
          headers: {
    ...
    const createUniqueServiceZoneForFulfillmentSet = async (
      ...
      const {
        ...
        input: {
          data: [
            {
              name: `Europe Store Return Shipping Options ${sellerLabel} ${unique}`,
    ...
    const setupSellerMarketplace = async (
      ...
      const seller = await createSellerAuthContext(
        `seller-store-return-shipping-${sellerLabel}-${unique}@example.com`,
        `Seller Store Return Shipping Options ${sellerLabel} ${unique}`
      )

      await createSellerShippingProfile(
        seller,
        `Store Return Shipping Options Shipping Profile ${sellerLabel} ${unique}`
      )
    ...
    const createProductVariantId = async (
      ...
      const {
        ...
        input: {
          products: [
            {
              title: `Store Return Shipping Options Product ${unique}`,
              handle: `store-return-shipping-product-${unique.replace(/_/g, '-')}`,
              status: 'published',
              sales_channels: [
                {
                  id: salesChannelId
    ...
    const createCart = async (
      ...
      const created = await cartService.createCarts({
        ...
        email: customer.email || `buyer-store-return-shipping-${unique}@example.com`,
    ...
    const completeCartWithListingAndReturnOrderId = async (
      ...
      const listing = await createListing(setup, {
        print_id: `print_store_return_shipping_options_${Date.now()}_${Math.floor(
          Math.random() * 10000
        )}`,
      ...
      const addListingResponse = await api.post(
        `/store/carts/${cart.id}/listings`,
        {
          ...
          quantity: 1,
          metadata: {
            source: 'store-return-shipping-options-test'
          }
        },
        {
    ...
    describe('Store Return Shipping Options API', () => {
      ...
      beforeEach(async () => {
        ...
        customerA = await createCustomerAuthContext(
          `customer-a-return-shipping-${uniqueSeed}@example.com`
        )

        customerB = await createCustomerAuthContext(
          `customer-b-return-shipping-${uniqueSeed}@example.com`
        )
      ...
      it('lists return shipping options for an authenticated customer own order', async () => {
        const response = await api.get('/store/shipping-options/return', {
          headers: {
      ...
      it('rejects return shipping options for another customer order', async () => {
        const response = await api.get('/store/shipping-options/return', {
          headers: {
            Authorization: `Bearer ${customerA.token}`,
            ...marketplaceBase.storeHeaders
