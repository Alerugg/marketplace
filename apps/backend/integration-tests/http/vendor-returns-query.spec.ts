...
medusaIntegrationTestRunner({
  ...
  testSuite: ({ api, getContainer }) => {
    ...
    describe('Vendor Returns Query API', () => {
      ...
      const getReturnIds = (returns: any[]) => {
        return returns.map((orderReturn: any) => {
          return orderReturn.return?.id ?? orderReturn.id
        })
      }

      const getReturnedIds = (payload: any) => {
        const returned = payload.return

        if (Array.isArray(returned)) {
          return returned.map((orderReturn: any) => orderReturn.id)
        }
      ...
      beforeEach(async () => {
        ...
        customerA = await createCustomerAuthContext(
          `customer-a-vendor-return-${uniqueSeed}@example.com`
        )

        customerB = await createCustomerAuthContext(
          `customer-b-vendor-return-${uniqueSeed}@example.com`
        )
      ...
      it('lists only returns that belong to the authenticated seller', async () => {
        ...
        const ids = getReturnIds(response.data.returns)

        expect(ids).toContain(sellerAReturnId)
        expect(ids).not.toContain(sellerBReturnId)
      })

      it('retrieves own return by id', async () => {
        const response = await api.get(`/vendor/returns/${sellerAReturnId}`, {
          headers: {
            Authorization: `Bearer ${sellerASetup.seller.token}`
          },
          validateStatus: () => true
        ...
        expectStatus(response, 200)
        expect(getReturnedIds(response.data)).toContain(sellerAReturnId)
      })

      it('rejects retrieving a return that belongs to another seller', async () => {
        const response = await api.get(`/vendor/returns/${sellerBReturnId}`, {
          headers: {
            Authorization: `Bearer ${sellerASetup.seller.token}`
          },
          validateStatus: () => true
      ...
      it('requires authentication on list and detail routes', async () => {
        const listResponse = await api.get('/vendor/returns', {
          validateStatus: () => true
        })

        const detailResponse = await api.get(
          `/vendor/returns/${sellerAReturnId}`,
          {
            validateStatus: () => true
          }
        )
