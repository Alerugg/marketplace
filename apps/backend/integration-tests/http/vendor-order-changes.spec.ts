...
const defaultListingPayload = {
  print_id: 'print_vendor_order_changes_001',
  price_amount: 19.99,
  currency_code: 'eur',
  condition_code: 'near_mint',
  quantity_available: 1,
  status: 'active',
  seller_note: 'vendor order changes test listing',
  photos: ['https://example.com/a.jpg'],
  location_country: 'ES',
  shipping_profile_id: 'sp_vendor_order_changes_123'
}
  ...
  testSuite: ({ api, getContainer }) => {
    ...
    const createSellerAuthContext = async (
      ...
      const seller = await sellerService.createSellers({
        name: sellerName,
        handle: `vendor-order-changes-seller-${unique}`,
    ...
    const setupSellerMarketplace = async (
      ...
      await createSellerShippingProfile(
        seller,
        `Vendor Order Changes Shipping Profile ${sellerLabel} ${unique}`
      )
    ...
    const createProductVariantId = async (
      ...
      const {
        ...
        input: {
          products: [
            {
              title: `Vendor Order Changes Product ${unique}`,
              handle: `vendor-order-changes-product-${unique.replace(/_/g, '-')}`,
              status: 'published',
              sales_channels: [
                {
                  id: salesChannelId
    ...
    const createCart = async (setup: MarketplaceSetup) => {
      ...
      const created = await cartService.createCarts({
        ...
        email: `buyer-vendor-order-changes-${unique}@example.com`,
    ...
    const completeCartWithListingAndReturnOrderId = async (
      ...
      const listing = await createListing(setup, {
        print_id: `print_vendor_order_changes_${Date.now()}_${Math.floor(
          Math.random() * 10000
        )}`,
      ...
      const addListingResponse = await api.post(
        `/store/carts/${cart.id}/listings`,
        {
          ...
          quantity: 1,
          metadata: {
            source: 'vendor-order-changes-test'
          }
        },
        {
    ...
    describe('Vendor Order Changes API', () => {
      ...
      it('allows the authenticated seller to list changes for their own order', async () => {
        const response = await api.get(
          `/vendor/orders/${sellerAOrderId}/changes`,
          {
            headers: {
        ...
        response.data.order_changes.forEach((orderChange: any) => {
          expect(orderChange.order_id).toBe(sellerAOrderId)
        })
      })

      it('allows listing changes after the seller cancels their own order', async () => {
      ...
      it('rejects listing changes for an order that belongs to another seller', async () => {
        const response = await api.get(
          `/vendor/orders/${sellerBOrderId}/changes`,
          {
            headers: {
              Authorization: `Bearer ${sellerASetup.seller.token}`
            },
      ...
      it('requires authentication on order changes route', async () => {
        const response = await api.get(
          `/vendor/orders/${sellerAOrderId}/changes`,
          {
            validateStatus: () => true
